...
\hypersetup{pdftitle={Hare-core nonemptiness for approval committees with two omissions},pdfauthor={Jiarui Fang},pdfsubject={Approval-based committee elections and coalitional stability},pdfkeywords={approval voting, Hare core, PAV, core existence}}
\newtheorem{theorem}{Theorem}
\newtheorem{lemma}[theorem]{Lemma}
\newcommand{\PAV}{\operatorname{PAV}}
\newcommand{\gain}{\operatorname{Q}}
\title{Hare-core nonemptiness for approval committees\\with two omissions}
...
finite approval election with $m\ge3$ candidates and $k=m-2$ seats has an
ordinary deterministic Hare-core committee, without restricting the
number of voters or their approval sets. The proof bounds repeated
...
two omissions. Every core of the residual election lifts to a core of
the original election. Induction gives a finite exact construction,
including quota equality and overlapping deviations. The result concerns
all candidate counts with two omissions, rather than a bounded range of
committee sizes; it does not establish existence for unrestricted $(m,k)$.
\end{abstract}
...
for each additional approved representative. Peters proved core existence
for $k\le8$ and, using a recursive PAV
...
The result here instead fixes the number of unselected candidates:
$m-k=2$, with no upper bound on $m$, $k$ or the electorate.
...
The proof is analytic. It does not use a finite classification,
linear-programming certificate or a fractional-to-integral passage.
We give the model and theorem first, establish the overlap bound,
prove the contraction, and conclude by induction. An exact example
then illustrates a contraction from a blocked PAV maximizer.
...
blocks a committee $W$ by a target $T\subseteq C$ if
\[
...
\begin{equation}\label{eq:core}
 W\text{ is core}\quad\Longleftrightarrow\quad
 kq(W,T)<n|T|\quad
 \text{for every }\varnothing\ne T\subseteq C,\ |T|\le k.
\end{equation}
All quota tests below use this integer convention. Equality is blocking.
No assumption of unanimous approval of a target is made, and targets may
overlap the current committee.

\begin{theorem}\label{thm:main}
For every finite approval election with $m\ge3$ candidates and $k=m-2$ seats,
an ordinary deterministic Hare-core committee exists.
\end{theorem}
...
$u=u_i(W)>0$, the sum is $-|A_i\cap Y|/u\ge-1$. If $u=0$, the sum is zero.
Thus this sum is at least $-1$ for every voter; no division by zero is used.

We reprove the following exclusion of Peters~\cite[Lemma~3.1(ii)]{Peters2025}.

\begin{lemma}\label{lem:one}
A Hare blocker of a one-swap PAV local maximizer cannot introduce exactly
one new candidate. In particular, if exactly two candidates are unselected,
every blocker contains both of them.
\end{lemma}
\begin{proof}
Let $W$ be such a local maximizer and suppose $T\setminus W=\{b\}$.
...
\]
If $T$ blocks, $kq\ge nt$ and $q>0$, so this expression is positive.
Local maximality says each swap has nonpositive total change, a
contradiction. A target contained in $W$ has no strict gainers. When two
candidates are unselected, a blocker must consequently contain both.
\end{proof}

The next estimate holds for \emph{every} completion of a first blocking
target. Neither the later committee nor the later target needs to optimize
an objective.

\begin{lemma}[Repeated-support bound]\label{lem:repeat}
Let $|C|=k+2$, let $W_0$ be a one-swap PAV local maximizer, and let $F$
be a Hare blocker of $W_0$. Put $Q_0=\gain(W_0,F)$. For any $k$-committee
...
and $T$ contains all its approvals. Lemma~\ref{lem:one} gives $B\subseteq F$,
...
This is also a finite construction: enumerate committees to find a global
PAV maximizer; enumerate all nonempty targets of size at most the current
seat count to check \eqref{eq:core}; if a blocker exists, contract as in
Lemma~\ref{lem:lift}, solve the smaller instance, and lift. The positive
integer seat count strictly decreases at each recursive call. Any choice
among tied PAV maximizers and any first blocker is allowed. No efficient
running-time claim is made.
...
well-founded induction. There is no fractional-to-integral passage.
...
\]
Under the same Hare-core and unique-global-PAV conventions, this example
has fewer voters than the $27$-voter example described as voter-minimal
in Peters~\cite[Section 4.3]{Peters2025}. We make no minimality claim
for eighteen voters.
...
The contraction preserves the class $m-k=2$ and accepts any core of the
smaller election. In each recursive call, PAV is evaluated on projected
residual approval sets, with that residual election's seat count.
The lifting proof, rather than an identification of the two quotas,
connects this residual calculation to every blocking test in the
parent electorate. This distinguishes the argument from an assertion
that an arbitrary sequence of fixed-target reoptimizations cannot
overcommit seats.
...
does not establish core existence for unrestricted $(m,k)$. It also
makes no claim about weak Nash certificates or polynomial running time.
Its conclusion is a uniform, integral existence theorem for the entire
two-omission class, supported by a complete contraction-and-lifting
argument.

\section*{Code availability}
The accompanying package contains the exact exhaustive constructor,
a separate set-based verifier and the example in Section~\ref{sec:example}.
They use only Python's standard library. The theorem does not depend on
these diagnostic computations. Instructions and the input format are
given in the package README. The repository for public release is
\url{https://github.com/Baymax-ray/approval-core-two-omissions}.

\section*{AI use}
GPT-6 Astra (OpenAI) was used for proof exploration and checking,
verification-code development, manuscript drafting and revision.
